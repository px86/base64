...
{
    return memcmp(prefix, arg, strlen(prefix)) == 0;
}
            ...
            for (auto itr = m_options.begin(); itr != m_options.end(); ++itr) {
                if (itr->long_name && starts_with(itr->long_name, argv[i] + 2)) {
                    if (itr->requires_value) {
                        ...
                        itr->accept_value(nullptr);
                    break;
                }
                if (itr + 1 == m_options.end())
...
#define FIELD_WIDTH 20
    ...
    for (auto& i : m_positional_arguments)
        std::cout << ' ' << i.name;
    std::cout << '\n';
    ...
    for (auto& opt : m_options) {
        ...
        if (opt.short_name && opt.long_name)
            ss << "  --" << opt.long_name << ", -" << opt.short_name;
        ...
        if (opt_str.size() >= FIELD_WIDTH) {
            ...
            std::cout << std::setw(FIELD_WIDTH) << opt_str << opt.help_message << '\n';
    }
